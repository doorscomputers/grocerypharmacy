...
function POSCartItem({
  ...
  return (
    <View style={[styles.container, { paddingVertical: sp.xs, paddingHorizontal: sp.sm + 2 }]}>
      {/* LINE 1: Index + Product Name (Full Width) */}
      <View style={styles.topRow}>
        <View style={[styles.indexContainer, { width: indexSize, height: indexSize, borderRadius: indexSize / 2, marginRight: sp.sm }]}>
          <Text style={[styles.indexText, { fontSize: fs.caption }]}>{index}</Text>
        </View>
        <View style={styles.nameContainer}>
          <Text style={[styles.productName, { fontSize: fs.body }]} numberOfLines={1}>
      ...
      <View style={[styles.bottomRow, { paddingLeft: indexSize + sp.sm }]}>
        ...
        {/* Line Total */}
        <Text style={[styles.lineTotal, { fontSize: fs.body, marginRight: sp.sm }]}>₱{formatCurrency(lineTotal)}</Text>
